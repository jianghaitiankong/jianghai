    ...
    <div class="container">
      ...
      <div class="video-content">
        <div class="video-list" v-for="item in videoList" :key="item.mediaId">
          <!-- <img :src="item.image"/> -->
          <video :src="item.filePath" controls="controls" style="width:300px;height:214px"></video>
          <p>{{item.mediaName}}</p>
...
export default {
  ...
  methods: {
    queryData() {
      ...
      that.$request
        .post("media/query", data, that.token)
...
.video-list img:first-child {
  width: 300px;
  height: 214px;
  margin-bottom: 10px;
  border: 1px solid #f9f9f9;
  padding: 5px;
  box-shadow: 0 0 10px #0546b4;
}
.video-list img:last-child {
  position: absolute;
  top: 32%;
}
.video-list p {
