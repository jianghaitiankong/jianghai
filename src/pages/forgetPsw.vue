            ...
            <div class="m-vfAccount m-center">
                  ...
                  <div class="u-ipt">
                    <div class="ipt_wrap">
                      <input
                        ...
                        name
                        placeholder="手机号码"
                        class="ipt_input ipt_input_large ipt_input-success"
                        maxlength="11"
                        autocomplete="off"
                      >
                  ...
                  <div class="u-ipt">
                    <div class="ipt_wrap">
                      <input
                        ...
                        name
                        placeholder="输入短信验证码"
                        class="ipt_input ipt_input_small ipt_input-"
                        maxlength="6"
                        autocomplete="off"
                      >
                      <div class="ipt_extend">
              ...
              <div class="u-btn">
                <button
                  ...
                  :disabled="btnDis"
                  :class="{disStyle:btnDis,canStyle:!btnDis}"
                >提交</button>
              </div>
            </div>
          </div>
  ...
  methods: {
    ...
    sendSms: function() {
      ...
      if (/^1[34578]\d{9}$/.test(this.phone)) {
        ...
        that.$request
          .post("user/exitByPhone", data, that.token)
          .then(function(res) {
            if (res.data.success == "校验成功") {
              ...
              that.$request
                .post("user/sendSms", data, that.token)
                .then(function(res) {
                  console.log(res);
    ...
    forgrtPsw: function() {
      ...
      if (!/^(?=.*[a-z])(?=.*\d)[^]{8,20}$/.test(that.password)) {
      ...
      that.btnDis = true;
      that.$request
        .post("user/forgotPassword", data, that.token)
        .then(function(res) {
          if (res.data.success == "密码重置成功") {
            that.successMsg = "密码重置成功";
            that.open1();
            setTimeout(function() {
              that.$router.push({
                path: "/login"
